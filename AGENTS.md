...
| `docs/explanation/decisions/` | ADRs 001–022 — fuente de verdad de arquitectura                                         |
...
6. **`/code-review medium` antes de decir "listo", no después.** En F6.5 se corrió al final y encontró 9 hallazgos reales, incluido uno que ya se había reportado como arreglado. Uno por PR, antes de mergear, y otro sobre el **rango completo de la fase** al cerrarla (el diff acumulado contra el `main` de antes de empezar). Siempre `medium`: los niveles de arriba (`high`, `xhigh`, `max`, `ultra`) se cobran con créditos aparte de la suscripción, así que no son una opción — más cobertura se consigue con más superficie, no con más nivel.
