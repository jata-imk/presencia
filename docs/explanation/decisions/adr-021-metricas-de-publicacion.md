# ADR-021 · Métricas de publicación: un snapshot por post y bucket

**Decisión:** las métricas de una publicación viven en `post_metrics`, una fila por **post y bucket**, llaveada por `(user_id, network, platform_post_id, snapshot_at)`. El bucket es de ancho variable según la edad del post —cada hora las primeras 12, después cada 6, y espaciándose hasta los 30 días— y esa escalera vive en `frescura.ts`. _(Nació como una fila por post y DÍA, llaveada por `snapshot_date`; el addendum del 2026-09-20, al final, cuenta por qué cambió. El resto de lo que sigue vale para las dos versiones.)_ La card es una referencia opcional, no la llave. Cada fila guarda las métricas dos veces: normalizadas en columnas (`impressions`, `reach`, `likes`, `comments`, `shares`) y crudas en `raw`. `NULL` significa "la red no lo reportó" y nunca se traduce a `0`.
...
- **No por cuenta conectada.** Dos motivos independientes. `social_account_id` es nullable —es `SET NULL`, para que desconectar no borre historial— y un `NULL` no colisiona en un índice único, así que el upsert insertaría una fila nueva en cada pase. Y la fila tampoco es estable: reconectar sin borrar reutiliza la misma (`claimConnectIntent` captura la violación de unicidad y reactiva), pero borrar la cuenta y volver a conectarla crea una nueva, y ahí el mismo post del mismo bucket se guardaría dos veces. `social_account_id` se conserva como dato, pero no identifica nada.
- **Un snapshot por bucket, no una fila viva.** Una fila que se sobrescribe pierde la velocidad, que es justo la señal: cuánto creció un post en sus primeras 24 h distingue el que funcionó del que no, y el total acumulado no. Guardar la serie completa por pase iría al otro extremo: las redes reportan con horas de retraso, así que cuatro pases diarios darían cuatro filas casi idénticas. La granularidad tiene que seguir a la velocidad con la que el dato cambia, que es alta las primeras horas y baja después — de ahí la escalera. El índice único hace que el segundo pase del mismo bucket actualice en vez de insertar — es el invariante que el DoD de F8.7 pide por nombre.
