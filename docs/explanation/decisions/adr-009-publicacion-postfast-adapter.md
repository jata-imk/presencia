...
**Cuál de las tres condiciones carga el peso, porque no es la que parece.** Con Upload-Post el `provider_ref` no cambia nunca (`PATCH`, mismo `job_id`). Con PostFast sí cambia, pero es **lo último que pasa**: `markRescheduled` mueve el `scheduled_at` en su transacción, después vienen el create y el cancel —hasta 30s de red— y recién al final se estampa la ref nueva. O sea que durante toda esa ventana la fila todavía tiene la ref vieja y el par coincide igual que con Upload-Post.

Lo primero que se mueve, en los dos proveedores, es el `scheduled_at`. Y no puede fallar como señal porque `MIN_LEAD_MS` obliga a que toda programación quede al menos 5 minutos en el futuro, mientras el cutoff del barrido es `now - 2 min`: cualquier reprogramación saca la card del rango por construcción. El par `(id, provider_ref)` es cinturón además de tirantes —cuesta nada y protege si algún día `MIN_LEAD_MS` baja por debajo del margen o aparece un camino que estampe una ref sin mover la hora—, pero **quitar el `scheduled_at` para "simplificar" reabriría el agujero completo**.

Un caso que parece un falso positivo y no lo es: cuando una reprogramación falla con rechazo explícito, `schedule()` restaura la hora vieja, y la fila vuelve a quedar `(scheduled, ref vieja, hora pasada)`. La guardia vuelve a pasar, y está bien: la card quedó exactamente en el estado que el barrido observó, así que lo que el proveedor dijo de esa ref sigue siendo válido.
...
## Addendum (2026-09-18, F8.7) — `getPostMetrics`, el sexto método del puerto

El puerto suma `getPostMetrics(posts)`, que devuelve un `Map` de `platformPostId` a snapshot. Dos decisiones de forma, y el estado de verificación de cada adapter, que esta vez es dispar de verdad.

**El puerto pregunta por post, no por card, y la llave es el id en la RED.** `provider_ref` identifica el envío dentro del proveedor; los tres endpoints de analíticas preguntan por el id nativo de la publicación. Por eso F8.7 empieza persistiendo `publication_cards.platform_post_id` (migración `0023`), que ya venía en el `HistoryItem` de Upload-Post y se estaba tirando.

**El troceo y los límites de tasa son del adapter, no del caller.** No es un detalle de implementación: los tres proveedores preguntan de forma incompatible. Upload-Post pide **una request por post** (tope de 100 cada 5 minutos, el único rate limit de toda su spec); PostFast pide **una por rango de fechas** y devuelve todo lo publicado en la ventana, sin paginar. Un caller que intentara trocear "bien" para los dos terminaría con la unión de las dos restricciones y lo peor de cada una.

**Tres resultados, y los tres son normales:** snapshot con números; snapshot con los cinco en `null` y el motivo en `raw`; o ausente del `Map`. La diferencia entre los dos últimos es qué sabemos: "preguntamos y la red dijo que no" es un hecho sobre esa publicación y se guarda (evita volver a gastar cuota el mismo día); "no llegamos a preguntar" no afirma nada y se reintenta.

**Corrección del review de fase (2026-09-19), porque la frontera entre esos dos casos estaba mal trazada.** Las redes que el endpoint del proveedor no cubre —X en Upload-Post— se estaban dejando ausentes, con el argumento de que no habíamos preguntado. Era una trampa: sin fila, `lastSnapshotDates` no las conoce nunca, así que la política de frescura las trata como "nunca medidas", que es la **máxima** prioridad, en todos los pases para siempre. Tres posts de X bastaban para llenar el cupo de un usuario y dejar sus posts de Facebook sin medir indefinidamente. Ahora dejan fila con el motivo: que el proveedor no cubra una red es un hecho que sabemos, y lo transitorio (la ausencia) queda reservado para lo que de verdad conviene reintentar.

**Y un cuarto camino, que no es un resultado sino una señal:** si TODO lo que se pidió falló, el adapter **lanza** en vez de devolver un Map vacío. Un Map vacío significa "no había nada que medir", que es indistinguible de "el proveedor está caído o la API key fue revocada" — y esa confusión dejaría el job marcado como completado cada 6 h mientras la tabla no crece. Es el modo de falla más caro de esta fase, porque la deuda de datos no se rebobina.

**Estado de verificación, que conviene no perder de vista:**

- **Upload-Post — verificado, sobre un endpoint que no existe en su documentación.** `GET /uploadposts/post-analytics` no está en su `openapi.json` (43 paths) ni en el sitemap de sus docs (79 páginas): solo se lo menciona de pasada dentro de la descripción de su variante cacheada. Existe. Todo su contrato se conoce por la sonda del 2026-09-17 contra la cuenta real, y si cambia no habrá aviso en ninguna spec. De ahí salió también el hallazgo que más moldeó el diseño: **`success: true` no implica que haya métricas** — cuando la red no las da, en vez de `post_metrics` viene `post_metrics_error`. Ramificar por `success` habría guardado filas de ceros para todo LinkedIn personal.
- **La variante `/cached` no sirve como fuente.** Es un write-through que se llena como efecto de las lecturas live: antes de la sonda devolvía `posts: []` con `published_posts_in_range: 3`; después traía exactamente una entrada, la del único post que había dado números. Leerla en vez del live sería leer nuestro propio eco.
- **PostFast — sin verificar, y hay que decirlo cada vez.** Su `GET /social-posts/analytics` está bien documentado (rango de fechas, `latestMetric`, `fetchedAt`, contadores como bigint serializado en string), pero la cuenta no tiene suscripción desde F7 y ese endpoint pide plan Growth. Está implementado contra la doc y cubierto solo por tests con `fetch` stubeado. Es la misma situación en la que quedó su emulación de `reschedule` en F7.5 — donde el review encontró un fallo real que ningún test stubeado había visto. Al recuperar la cuenta se revisa antes de confiar en sus números.
- **fake — deterministas a propósito.** Los números salen de un hash del `platformPostId` y crecen con la antigüedad del post, así que el mismo post devuelve siempre lo mismo, también entre reinicios. Con `Math.random()` cada pase "descubriría" métricas nuevas y un test de "el segundo pase no duplica" pasaría por azar. Uno de cada cinco devuelve el caso sin métricas: en producción ese camino es la norma, y si el fake no lo recorriera, el primer lugar donde aparecería sería producción.

**Lo que sigue siendo cierto de ADR-009:** `CardsService` no cambió una línea por esto, y el puerto sigue sin filtrar nada específico de un proveedor. La asimetría entre proveedores no está en la interfaz, está en la cobertura de redes: LinkedIn solo da métricas de páginas de empresa (limitación de la API de LinkedIn, no de los proveedores — los tres lo dicen igual), y X no lo cubre el endpoint de Upload-Post.
