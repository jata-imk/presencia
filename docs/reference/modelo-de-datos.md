...
| `vertical`                                   | text     | id del catálogo cerrado (`packages/shared/src/verticals.ts`). De acá cuelga la caché compartida de tendencias, que se llavea por `(vertical, país, macro-región)` y no por `user_id`. `NULL` = "el usuario no la eligió" y se deriva de `niche` al leer — por eso no hay backfill. La macro-región **no** se guarda: se deriva de `market_region` con una función pura, para que no pueda quedar desincronizada de su fuente. Migración 0028 (F9) |
