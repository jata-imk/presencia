...
| `vertical`                                   | text     | id del catálogo cerrado (`packages/shared/src/verticals.ts`). Nació como llave de la caché compartida de tendencias; desde F9.6 esa caché no existe (ADR-024) y la vertical quedó como **default sugerido y etiqueta de contexto**: entra al prompt de búsqueda y es el sujeto del estado vacío, pero no decide qué fila se lee. `NULL` = "el usuario no la eligió" y se deriva de `niche` al leer — por eso no hay backfill. La macro-región **no** se guarda: se deriva de `market_region` con una función pura, para que no pueda quedar desincronizada de su fuente. Migración 0028 (F9) |
| `trend_prompt` / `trend_exclude`             | text     | qué quiere que busquemos y qué no quiere ver, en sus palabras. Opcionales: sin ellos la búsqueda se arma con el nicho, la audiencia y la región. Entran al prompt como DATO delimitado, nunca como instrucción (ADR-024). Migración 0034 (F9.6)                                                                                                                                                                                                                                                                                                                                              |
| `trend_langs`                                | text[]   | idiomas de las fuentes, default `{es}`. En nichos técnicos casi todo lo que se mueve está en inglés, y el prompt fijo lo escondía. Migración 0034 (F9.6)                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `modo`                                       | enum     | `crecer` \| `mantener` \| `lanzar` — el objetivo activo que Ritmo muestra como "Modo". Mueve la meta semanal sugerida y entra al payload de la narración; si no cambiara nada sería un chip decorativo. `NULL` = "el usuario no lo eligió" y se deriva de `extras.goals` al leer, mismo criterio que `vertical`. Migración 0032 (F9.5)                                                                                                                                                                                                                                                       |
...
| `extras`                                     | jsonb    | lo que el onboarding aprenda después sin migrar. `extras.goals` (paso "Goals") dejó de ser dato muerto en F9.5: de ahí sale el default de `modo`                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| `platform_post_id`              | text nullable    | id del post en la RED, no en el proveedor (F8.7, migración `0023_cards_platform_post_id`); llave con la que se le piden métricas — `provider_ref` no sirve para eso |
...
| `reason`                          | enum               | `monthly_grant`, `cycle_expiration`, `chat_message`, `idea_generation`, `multi_adapt`, `image_generation`, `weekly_calendar`, `ritmo_narration`, `trend_refresh`, `refund`, `adjustment` |
...
Es deliberadamente angosta: **solo `SELECT`** (toda escritura sigue pasando por `runWithTenant`), **solo `scheduled`** (una card en `draft`, `published` o `failed` sigue invisible fuera de su tenant), **solo lo accionable hoy** (una card programada para dentro de dos semanas no va a cambiar de estado en el pase de hoy), y solo en modo barrido.

Ese último recorte respeta las dos ramas de la reconciliación, y el detalle no es obvio: las **huérfanas** no llevan `provider_ref` y su `scheduled_at` puede estar en el futuro —el proceso murió al programarlas, el proveedor nunca las recibió, no se van a publicar nunca— así que hay que poder cerrarlas igual. Acotarlas por fecha las dejaría colgadas para siempre. Y el corte es `now()` a secas, no `now() - RECONCILE_GRACE_MS`: el margen de gracia vive en el código, y copiarlo acá crearía la misma regla en dos lugares. Con la policy deliberadamente un poco **más ancha** que la consulta, no puede esconder una fila que el pase sí pide — solo recorta lo que sobra.

La salvedad: el `cutoff` de la consulta lo calcula el proceso y este `now()` lo calcula Postgres. Si el reloj de la app se adelantara al de la base por más que el margen de gracia, una card recién vencida podría quedar del lado invisible por esa diferencia — se recupera sola en el pase siguiente, así que el efecto es un retraso del tamaño del desfase, no una card colgada. En prod los dos relojes son el de la misma máquina.
...
**`user_trends`** — la tanda de tendencias de cada persona (F9.6, ADR-024).

| Columna                       | Tipo        | Nota                                                                                                                                                                                                                      |
| ----------------------------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `user_id`                     | uuid FK     | RLS. Único: una tanda vigente por usuario, el refresco es un upsert                                                                                                                                                       |
| `generated_at` / `expires_at` | timestamptz | `expires_at` explícito y no derivado de un TTL: así una tanda mala se invalida a mano sin tocar código. El barrido ordena por él                                                                                          |
| `items`                       | jsonb       | array de `TrendItem` (`packages/shared/src/trends.ts`), validado con Zod al leer                                                                                                                                          |
| `provider` / `model`          | text        | quién la produjo                                                                                                                                                                                                          |
| `usage`                       | jsonb       | tokens, duración y **`consultas`**: cuántas búsquedas disparó la llamada. El fee del grounding se cobra por consulta y una llamada puede lanzar varias, así que es el número del que cuelga cualquier proyección de costo |

**`trend_sources`** — los medios que el usuario quiere que miremos (F9.6).

| Columna   | Tipo    | Nota                                                                                                                               |
| --------- | ------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| `user_id` | uuid FK | RLS. Único junto con `host`                                                                                                        |
| `host`    | text    | el host ya normalizado, sin esquema ni `www.`. Es lo que la búsqueda acota con `site:`; guardar la ruta acotaría a una sola página |

**`trend_refreshes`** — cada vez que alguien adelanta su refresco (F9.6).

Existe por tres razones, y ninguna es "guardar historial":

| Columna      | Tipo        | Nota                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ------------ | ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `id`         | uuid PK     | **Es el `reference_id` del asiento.** `user_trends.id` no sirve: esa fila es un upsert, su id no cambia entre refrescos y el segundo cobro chocaría contra `ledger_dedup` y se perdería en silencio                                                                                                                                                                                                                                                  |
| `user_id`    | uuid FK     | RLS                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `billable`   | boolean     | Si se iba a cobrar, **decidido al pedirlo**. La pregunta ("¿tenía tendencias vigentes?") solo se puede contestar en el momento del click: cuando el job termina, ~40s después, la tanda ya es otra                                                                                                                                                                                                                                                   |
| `settled_at` | timestamptz | Null mientras el job corre, y ese null **es el candado**: un índice único parcial sobre `user_id where settled_at is null` deja un solo refresco en vuelo por persona. Contra el doble click vale un índice, no un `if` que puede perder la carrera. Y tiene salida: las filas más viejas que el doble del techo del job se cierran como `abandonado` al leer el estado, porque pg-boss no mata al handler al expirar y nadie volvería a liquidarlas |
| `outcome`    | text        | `cobrado`, `gratis`, `sin_resultados`, `error` o `no_encolado`. Se lee cuando algo se ve raro; nadie filtra por esto                                                                                                                                                                                                                                                                                                                                 |

**Se cobra solo adelantar tendencias que el usuario ya tiene.** Sin tanda, con la tanda vencida o con una vacía —lo que deja una búsqueda que no encontró nada citable— el refresco es gratis: cobrar por la primera entrega, o por reintentar lo que no dio resultados, es cobrar por lo que no se recibió. La regla vive sola en `apps/api/src/trends/cobro.ts` y tiene su propio spec.

RLS activo en: `brand_voices`, `folders`, `chats`, `messages`, `publication_cards`, `assets`, `channel_links`, `social_accounts`, `social_connect_intents`, `credit_ledger`, `ai_usage_events`, `post_metrics`, `cadence_targets`, `ritmo_narrations`, `user_trends`, `trend_sources`, `trend_refreshes`. **Ya no hay excepciones**: `niche_trends` era la única tabla del dominio sin `user_id` —una caché compartida por `(vertical, país, macro-región)`, justificada en ADR-023— y se eliminó en F9.6 al pasar las tendencias a ser por usuario (ADR-024). `user_trends` lleva además una policy de worker acotada a `SELECT` y al centinela del barrido, con el mismo patrón que `worker_metrics_scan`: el barrido necesita preguntar cross-tenant a quién le toca refresco, y sin esa policy devolvería cero filas y las tendencias no se refrescarían nunca, sin un solo error. Las tablas de Better Auth se administran con su propio contrato (la librería filtra por sesión); evaluar RLS ahí como capa extra en F13 (hardening).
