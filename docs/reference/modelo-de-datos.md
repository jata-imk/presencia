...
| `settled_at` | timestamptz | Null mientras el job corre, y ese null **es el candado**: un índice único parcial sobre `user_id where settled_at is null` deja un solo refresco en vuelo por persona. Contra el doble click vale un índice, no un `if` que puede perder la carrera |
