...
- **`motion`** (paquete renombrado de framer-motion, `apps/web/src/lib/motion.ts`) para entrada/salida del DOM y cambios de layout: drawer, toasts, modales, cards nuevas. `DURATION`/`EASE_OUT`/`EASE_IN_OUT` en ese archivo son el espejo en JS de `--duration-*`/`--ease-*` — motion no puede leer `var()` de CSS. Variants compartidas: `fadeUp`, `drawerPush`, `sheetUp`, `sheetLeft` (drawer mobile del sidebar), `collapseSection` (acordeón de carpetas), `toastIn`, `backdropFade`.
